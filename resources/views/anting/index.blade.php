...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <div class="row">
                <div class="col-10">
                    <h4 class="card-title">List Anting</h4>
                </div>
                <div class="col-2">
                    <a class="btn btn-sm btn-custom float-end" href="{{ url('/anting/create') }}">Tambah</a>
                </div>
            </div>
        </div>
        <div class="card-body">
                        ...
                        @foreach ($databarang as $db)
                            <tr>
                                ...
                                <td class="text-center">
                                    <a class="btn btn-sm btn-custom" href="{{ url('/anting/' . $db->id . '/edit') }}">Ubah</a>
                                    <a class="btn btn-sm btn-custom" href="{{ url('/anting/' . $db->id) }}">View</a>
                                    <form style="display: inline;" action="{{ url('/anting/' . $db->id ) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
